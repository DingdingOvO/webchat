  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        }} type="text" placeholder="群组名称" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
...
function ChatPageInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const pData = await (pRes.ok ? pRes.json() : []);
        setFriends(fData);
        setGroups(gData);
  ...
  async function handleSearch(q: string) {
    setSearchQuery(q);
    if (!q.trim() || !auth) { setSearchResults([]); return; }
    try {
            ...
            <div className={styles.requestActions}>
              {pendingRequests.map((r: any) => (
                <div key={r.id} className={styles.requestItem}>
                  ...
                  <button className={styles.requestAccept} onClick={() => handleRequestAction(r.id, 'accept')}><CheckmarkIcon size={14} /></button>
                  <button className={styles.requestReject} onClick={() => handleRequestAction(r.id, 'reject')}><DismissIcon size={14} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={styles.list}>
          {tab === '消息' && (
            conversations.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text-faint)', fontSize: 'var(--fs-sm)', padding: 'var(--p8) var(--p4)' }}>
                暂无消息
              </div>
            ) : (
                  ...
                  {groups.map((g) => {
                    ...
                    return (
                      <div key={g.id} className={`${styles.contactItem} ${ck === activeConvKey ? styles.contactItemActive : ''}`}
                        onClick={() => selectConv(ck)}>
                        <div className={styles.contactAvatar} style={{ background: 'var(--gray-500)' }}>
